        ...
        if (collateralAddress && quoteTokenAddress) {
          ...
          if (debtToken && collateralToken) {
            ...
            for (const log of logs) {
              ...
              if (
                Object.values(AjnaEventSignatures).indexOf(signature) !== -1 &&
                compareAddress(address, poolAddress)
              ) {
                ...
                switch (signature) {
                  ...
                  case AjnaEventSignatures.Take: {
                    ...
                    volumeCollateralLiquidated = volumeCollateralLiquidated.plus(
                      new BigNumber(
                        formatBigNumberToString(event.args.collateral.toString(), SolidityUnits.WadDecimals),
                      ),
                    );
                    break;
                  }
