...
export default class BalancerFlashloanAdapter extends FlashloanProtocolAdapter {
  ...
  public async getFlashloanDataTimeframe(
    ...
    for (const log of logs) {
      ...
      if (signature === BalancerEventSignatures.Flashloan) {
        ...
        if (token) {
          ...
          const feesAmount = new BigNumber(formatBigNumberToString(event.args.feeAmount.toString(), token.decimals));

          const executor = normalizeAddress(event.args.recipient);
